...
import com.github.geng.constant.FeignConstants;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (authorizeIgnore.isMatch(httpRequest.getRequestURI())) {
            ...
            if (!StringUtils.hasText(userAuthToken)) {
                ...
                try {
                    // 验证token 接口
                    authenticationSerious.authentication(userAuthToken, httpRequest);
                    // 如果不是微服务，这里可以调用 UsernamePasswordAuthenticationToken 进行权限验证
                    // 但是用户权限已经在api网关进行验证
                    // 所以这里不需要验证
                    chain.doFilter(request, response);
                } catch (Exception e) {
...
//            // 微服务间的远程调用
//            if (FeignConstants.FROM_CLIENT.equals(httpRequest.getHeader(FeignConstants.IS_CLIENT))) {
//                log.debug("服务端远程调用鉴权开始");
//                //获取当前服务可访问列表，与请求微服务名称比较
//                // List<String> allowedClient = clientAuthSchedule.getAllowedClient();
//                String headerClientName = httpRequest.getHeader(FeignConstants.CLIENT_NAME);
//
